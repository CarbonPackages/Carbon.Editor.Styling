...
export default function BorderRadiusBox({ selected = null, organic = false, style = {}, className }) {
    ...
    const opacity = useMemo(() => (!selected || selected === "all" ? 1 : 0.2), [selected]);
    const topLeft = useMemo(() => (selected === "topLeft" ? 1 : opacity), [selected, opacity]);
    const topRight = useMemo(() => (selected === "topRight" ? 1 : opacity), [selected, opacity]);
    const bottomRight = useMemo(() => (selected === "bottomRight" ? 1 : opacity), [selected, opacity]);
    const bottomLeft = useMemo(() => (selected === "bottomLeft" ? 1 : opacity), [selected, opacity]);
